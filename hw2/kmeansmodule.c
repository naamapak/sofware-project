...
PyObject* clusters_to_list(cluster* result, int k, int dimension) {
    PyObject* py_centroid_list = PyList_New(k);
    for (int i = 0; i < k; i++) {
        PyObject* centroid_vec = PyList_New(dimension);
        for (int j = 0; j < dimension; j++) {
            PyObject* val = PyFloat_FromDouble(result[i].vector[j]);
            PyList_SET_ITEM(centroid_vec, j, val);
        }
        PyList_SET_ITEM(py_centroid_list, i, centroid_vec);
    }
...
PyObject* fit(PyObject* self, PyObject* args){
   ...
   if(!PyArg_ParseTuple(args, "OOiid", &py_clusters, &py_data, &k, &iter, &epsilon)){
        return NULL;
    }
    ...
    py_result = clusters_to_list(result, k, result[0].vector_size);
